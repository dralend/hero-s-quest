@tool
@icon("res://assets/enemies/enemies/icons/enemy.svg")
...
func setup() -> void:
	...
	for c in get_children():
		if c is AnimationPlayer and not animation:
			...
			state_machine = c
		elif DecisionEngine and not decision_engine:
			decision_engine = c
		pass
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if not find_children("*", "DamageArea", false):
		warnings.append("Requires an DamageArea node!")
	if not find_children("*", "HazardArea", false):
		warnings.append("Requires an HazardArea node!")
	...
	if not find_children("*", "DecisionEngine", false):
		warnings.append("Requires an DecisionEngine node!")
