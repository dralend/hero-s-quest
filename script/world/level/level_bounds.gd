@tool 
@icon ("res://assets/world/level_bounds.svg")
class_name LevelBounds extends Node2D

@export_range(480, 3200, 32, 'suffix:px') var width: int = 480: set = _on_width_changed
@export_range(270, 3200, 32, 'suffix:px') var height: int = 270: set = _on_height_changed
...
func _draw() -> void:
	if Engine.is_editor_hint():
		...
		draw_rect(r, Color(0.0, 0.45, 1.0, 0.6), false, 9)
