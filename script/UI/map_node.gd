...
func _ready() -> void:
	if Engine.is_editor_hint():
		...
		if not SaveManager.is_area_discoverd(linked_scene):
			visible = false
...
func update_node() -> void:
	...
	var transition: Array[leveltransition] = []
	...
	size = size.round()
	create_entrance_data(transition)
	create_transition_blocks()
	pass
...
func update_node_label(scene: Node) -> void:
	...
	t = t.replace("res://scenes/world/levels/", "")
	t = t.replace(".tscn", "")
	label.text = t
	pass


func create_entrance_data(transition: Array[leveltransition]) -> void:
